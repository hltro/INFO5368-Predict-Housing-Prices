...
def split_dataset(X, y, number):
    ...
    try:
        ...
        # Add code here
        X_train, X_val, y_train, y_val = train_test_split(X, y, test_size = number/100, random_state=45)
...
def inspect_coefficients(models, inspect_models):
    ...
    for model in inspect_models:
        if model == 'Multiple Linear Regression':
            coeff = models[model].coef_
            out_dict['Multiple Linear Regression'] = coeff
            st.write('Coefficients of Multiple Linear Regression are: ', str(coeff))
        elif model == 'Polynomial Regression':
            coeff = models[model][-1].coef_
            out_dict['Polynomial Regression'] = coeff
            st.write('Coefficients of Polynomial Regression are: ', str(coeff))
        elif model == 'Ridge Regression':
            coeff = models[model][-1].best_estimator_.coef_
            out_dict['Ridge Regression'] = coeff
            st.write('Coefficients of Ridge Regression are: ', str(coeff))
        elif model == 'Lasso Regression':
            coeff = models[model][-1].best_estimator_.coef_
            out_dict['Lasso Regression'] = coeff
            st.write('Coefficients of Lasso Regression are: ', str(coeff))
...
def train_multiple_regression(X_train, y_train, regression_methods_options):
    ...
    try:
        ...
        st.write("Error: Cannot perform training on Multiple Linear Regression.")

    if 'Multiple Linear Regression' not in st.session_state:
        st.session_state['Multiple Linear Regression'] = None
...
def train_polynomial_regression(X_train, y_train, regression_methods_options, poly_degree, poly_include_bias):
    ...
    try:
        if 'Polynomial Regression' in regression_methods_options:
            from sklearn.preprocessing import PolynomialFeatures
            poly_reg_pipeline = Pipeline([('scaler', StandardScaler()),
                                       ('poly', PolynomialFeatures(degree=poly_degree, include_bias=poly_include_bias)),
                                       ('polyReg', LinearRegression())])
            poly_reg_pipeline.fit(X_train, y_train)
    except:
        st.write("Error: Cannot perform training on Polynomial Regression.")

    if 'Polynomial Regression' not in st.session_state:
        st.session_state['Polynomial Regression'] = None
        st.session_state['Polynomial Regression'] = poly_reg_pipeline
        
    return poly_reg_pipeline
...
def train_ridge_regression(X_train, y_train, regression_methods_options, ridge_params, ridge_cv_fold):
    ...
    try:
        ...
        st.write("Error: Cannot perform training on Ridge Regression.")

    if 'Ridge Regression' not in st.session_state:
        st.session_state['Ridge Regression'] = None
        st.session_state['Ridge Regression'] = ridge_cv_pipeline

    return ridge_cv_pipeline
...
def train_lasso_regression(X_train, y_train, regression_methods_options, lasso_params, lasso_cv_fold):
    ...
    try:
        ...
        st.write("Error: Cannot perform training on Lasso Regression.")

    if 'Lasso Regression' not in st.session_state:
        st.session_state['Lasso Regression'] = None
...
if df is not None:
    ...
    if (regression_methods_options[0] in regression_model_select):
        ...
        if st.button('Train Multiple Linear Regression Model'):
            multi_reg_model = train_multiple_regression(
    ...
    if (regression_methods_options[1] in regression_model_select):
        ...
        if st.button('Train Polynomial Regression Model'):
            poly_reg_model = train_polynomial_regression(
    ...
    if (regression_methods_options[2] in regression_model_select):
        ...
        if st.button('Train Ridge Regression Model'):
            ridge_cv = train_ridge_regression(
    ...
    if (regression_methods_options[3] in regression_model_select):
        ...
        if st.button('Train Lasso Regression Model'):
            lasso_cv = train_lasso_regression(
